# Study over 7000 foods in the USDA National Nutrient Database for Standard Reference.
...
# 2. Explore the Foods with high sodium
...
USDA$HighCarbs = as.numeric(USDA$Carbohydrate > mean(USDA$Carbohydrate, na.rm =
                                                       TRUE))
USDA$HighProtein = as.numeric(USDA$Protein > mean(USDA$Protein, na.rm =
                                                    TRUE))
USDA$HighFat = as.numeric(USDA$TotalFat > mean(USDA$TotalFat, na.rm =
                                                 TRUE))
